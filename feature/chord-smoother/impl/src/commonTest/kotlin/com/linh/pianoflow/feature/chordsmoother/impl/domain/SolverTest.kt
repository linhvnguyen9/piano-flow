...
class SolverTest {
    ...
    fun dp_equals_brute_force_C_G_Am_F() {
        ...
        val opt = totalMovement(solveVoiceLeading(cs, anchor = false))
    ...
    fun keyboard_range_contains_voicings_and_aligns_to_C_and_B() {
        val cs = chords("C | G | Am | F")
        val path = solveVoiceLeading(cs, anchor = true)
    ...
    fun solve_returns_empty_for_empty_input() {
        assertEquals(emptyList(), solveVoiceLeading(emptyList(), anchor = true))
    }
    ...
    fun pinned_voicing_is_honoured_and_others_reflow() {
        ...
        val pinned = solveVoiceLeading(cs, anchor = false, pins = mapOf(1 to pickIdx))
    ...
    fun pinning_does_not_break_no_pin_signature() {
        val cs = chords("C | G | Am | F")
        val a = totalMovement(solveVoiceLeading(cs, anchor = false))
        val b = totalMovement(solveVoiceLeading(cs, anchor = false, pins = emptyMap()))
